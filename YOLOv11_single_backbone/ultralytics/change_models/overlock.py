...
# 论文题目：OverLoCK: An Overview-first-Look-Closely-next ConvNet with Context-Mixing Dynamic Kernels CVPR2025
...
class OverLoCK(nn.Module):
    ...
    def forward(self, x):
        ...
        # return x
        out = []
        x = self.forward_pre_features(x)  # 1 112 80 80
        out.append(x)
        x, ctx = self.forward_base_features(x)
        out.append(x)
        out.append(ctx)
...
def main():
    parser = argparse.ArgumentParser(description='OverLoCK 模型快速推理')
    parser.add_argument('--model', type=str, default='overlock_xt',
                        choices=['overlock_xt', 'overlock_t', 'overlock_s', 'overlock_b'],
                        help='选择模型 (默认: overlock_xt)')
    ...
    # 生成归一化后的随机张量 (shape: [B, C, H, W])
    x = torch.rand(args.batch_size, 3, args.input_size, args.input_size).to(args.device)
    ...
    print(f"设备内存占用: {torch.cuda.memory_allocated() / 1e6:.2f} MB" if args.device == 'cuda' else "CPU模式")
    print("=" * 40)

    # 示例输出
    print("\n示例输出 (第一个样本):")
    print(f"最大概率: {torch.max(out).item():.4f}")
    print(f"最小概率: {torch.min(out).item():.4f}")
    print(f"前5个类别的索引: {torch.topk(out[0], 5).indices.cpu().numpy()}")
