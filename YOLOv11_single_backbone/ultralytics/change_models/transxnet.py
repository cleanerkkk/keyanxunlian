...
# 论文题目：TransXNet: Learning Both Global and Local Dynamics with a Dual Dynamic Token Mixer for Visual Recognition  TNNLS 2025
# 论文链接：https://arxiv.org/abs/2310.19380
# 代码改进者：一勺汤
...
class TransXNet(nn.Module):
    ...
    def forward_tokens(self, x):
        outs = []
        pos_idx = 0
        for idx in range(len(self.network)):  # 4 6
            if idx in [0, 2, 4, 6]:
...
@register_model
def transxnet_t(pretrained=False, **kwargs):
...
@register_model
def transxnet_s(pretrained=False,  **kwargs):
...
@register_model
def transxnet_b(pretrained=False, **kwargs):
...
def main():
    ...
    # 创建模型
    model = transxnet_t(pretrained=False, num_classes=num_classes) if model_type == 't' else \
        transxnet_s(pretrained=False, num_classes=num_classes) if model_type == 's' else \
            transxnet_b(pretrained=False, num_classes=num_classes)
    model = model.to(device)
    print(f"Created TransXNet-{model_type.upper()} model with {num_classes} classes")
    ...
    print(f"Sample class scores (first 5):\n{output[0, :5].cpu().numpy()}")
