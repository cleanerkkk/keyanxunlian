...
def triton_cross_scan_flex_k2(
    ...
    _mask_w = (i_w * BW + tl.arange(0, BW)) < DW
    # _mask_hw = _mask_h[:, None] & _mask_w[None, :]
    # 修复后代码（使用Triton原生维度操作）
    _mask_h_exp = tl.expand_dims(_mask_h, axis=1)  # 替代 [:, None]
    _mask_w_exp = tl.expand_dims(_mask_w, axis=0)  # 替代 [None, :]
    _mask_hw = _mask_h_exp & _mask_w_exp
    _for_C = min(DC - i_c * BC, BC)
    ...
    # 修改后代码
    range_bh = tl.arange(0, BH)
    range_bw = tl.arange(0, BW)

    # 使用 Triton 内置维度操作
    range_bh_2d = tl.expand_dims(range_bh, axis=1)  # 替代 [:, None]
    range_bw_2d = tl.expand_dims(range_bw, axis=0)  # 替代 [None, :]
    ...
    # 修复后代码（完全使用Triton类型系统）
    range_bh = tl.arange(0, BH)
    range_bw = tl.arange(0, BW)
    range_bh_2d = tl.expand_dims(range_bh, axis=1)  # 替换 [:, None]
    range_bw_2d = tl.expand_dims(range_bw, axis=0)  # 替换 [None, :]
