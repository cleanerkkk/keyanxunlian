...
class MobileMamba(torch.nn.Module):
    ...
    def forward(self, x):
        out=[]
        x = self.patch_embed(x) # 4 144 80 80
        x = self.blocks1(x) #  4 144 80 80
        out.append(x)
        x = self.blocks2(x) #  4 272 40 40
        out.append(x)
        x = self.blocks3(x) #  4 368 20 20
        out.append(x)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-i', '--imagesize', type=int, default=640)
    parser.add_argument('-m', '--modelname', default="MobileMamba_T2")
